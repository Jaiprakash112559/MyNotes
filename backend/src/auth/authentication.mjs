...
const authorization = async (req, res, next) => {
    try {
        ...
        const user = req.user || await userModel.findById(token.id);
        ...
        const isSelfParams = req.params.id === token.id;
        const isSelfBody = req.body?.email === user.email;

        if (isAdmin || isSelfParams || isSelfBody) {
            next();
        } else {
